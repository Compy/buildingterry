...
-(id) initWithFrame:(CGRect)frame {
    ...
    if (self) {
        ...
        indicator.frame = (CGRect){ frame.size.width / 2 - indicator.frame.size.width / 2,
                                    frame.size.height / 2 - indicator.frame.size.height / 2 - 10,
        ...
        self.textLabel = [[UILabel alloc] initWithFrame:CGRectMake(0, frame.size.height - 22 * 2, frame.size.width, 22)];
...
- (void)webViewDidFinishLoad:(UniWebView *)webView {
    ...
    if (!webView.isLoading) {
        webView.currentUrl = webView.request.URL.absoluteString;
    }
}
...
-(BOOL)webView:(UniWebView *)webView shouldStartLoadWithRequest:(NSURLRequest *)request navigationType:(UIWebViewNavigationType)navigationType {
    NSString *webViewName = [self webViewName:webView];
    if ([[request.URL absoluteString] rangeOfString:@"uniwebview"].location == 0) {
        NSString *rawMessage = [NSString stringWithFormat:@"%@?%@",[request.URL host],[request.URL query]];
        ...
        if (!webView.isLoading) {
            ...
            UnitySendMessage([webViewName UTF8String], "LoadBegin", "");
        }
    }
    return YES;
}
